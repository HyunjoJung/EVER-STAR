...
import com.everstarbackmain.global.vertexai.util.GeminiImageClient;
...
public class OpenAiClient {
	...
	public OpenAiClient(@Qualifier("openAiRestTemplate") RestTemplate restTemplate, OpenAiConfig openAiConfig, GeminiImageClient geminiImageClient) {
	...
	public String writePetTextToImageAnswer(Pet pet, Quest quest, QuestAnswer questAnswer) {
		...
		// Vertex AI Imagen으로 이미지 생성 (DALL-E 대체)
		log.info("main server - Generating image with Vertex AI Imagen: {}", prompt);
		return geminiImageClient.generateImageFromText(prompt);
	}
	...
	public String writePetTextImageToImageAnswer(Quest quest, QuestAnswer questAnswer, String imageUrl) {
		...
		// Vertex AI Imagen으로 이미지 생성 (DALL-E 대체)
		// GPT-4 Vision이 생성한 향상된 프롬프트와 원본 이미지를 함께 사용
		log.info("main server - Generating image with Vertex AI Imagen (text+image): {}", prompt);

		// 이미지 URL을 Base64로 변환해야 하는 경우 처리 필요
		// 현재는 imageUrl이 이미 적절한 형식이라고 가정
		return geminiImageClient.generateImageFromTextAndImage(prompt, imageUrl);
	}
